...
19 جدول إجمالًا بعد إضافة جداول إعدادات الولاء وسجل حركات النقاط — التفاصيل الكاملة لكل عمود موجودة في ملفات الـ Migrations نفسها.
...
عند إنشاء الطلب يمكن إرسال `redeemed_points` مع بيانات العميل. يظل `total_amount` هو إجمالي الأصناف قبل الخصم، بينما المبلغ المطلوب تحصيله هو `payable_amount = total_amount - redeemed_amount`. لا يمكن الاستبدال إلا للعميل المسجل، وبعد بلوغ الحد الأدنى والرصيد المطلوبين.

### Customer Loyalty (`auth:customer`)

| Method | Endpoint | الوصف |
|---|---|---|
| GET | `/api/customer/loyalty` | عرض الرصيد الحالي وسجل حركات النقاط المشترك بين كل الفروع |

يتم اكتساب النقاط عند تسجيل الدفع بنجاح، وليس عند إنشاء الطلب. ويتم حسابها على المبلغ المدفوع فعليًا بعد الخصم. إذا رُفض الطلب أو أُلغي بعد استخدام النقاط، يعيد النظام النقاط تلقائيًا ويسجل حركة `refund`.
...
- **الدفع يدوي بالكامل** (كاش/بطاقة) من غير بوابة دفع أونلاين، وبيتحقق من تطابق المبلغ مع `payable_amount` بعد خصم النقاط قبل ما يقفل الطلب.
- **إعدادات الولاء قابلة للتعديل من لوحة الأدمن**: `points_earn_rate` و`point_redemption_value` و`minimum_points_to_redeem` محفوظة في صف إعدادات واحد، ولا يوفّر موديول الطلبات endpoint لتعديلها.
